"""
main.py — Bot va Server bitta faylda ishga tushadi
...
def find_and_update(buttons, btn_id, new_data):
    for i, btn in enumerate(buttons):
        if btn["id"] == btn_id:
            buttons[i].update(new_data)
            return True
        if find_and_update(btn.get("children", []), btn_id, new_data):
            return True
    return False
...
def find_and_delete(buttons, btn_id):
    for i, btn in enumerate(buttons):
        if btn["id"] == btn_id:
            buttons.pop(i)
            return True
        if find_and_delete(btn.get("children", []), btn_id):
            return True
    return False
...
def find_parent_and_add(buttons, parent_id, new_btn):
    for btn in buttons:
        if btn["id"] == parent_id:
            btn.setdefault("children", []).append(new_btn)
            btn["type"] = "menu"
            return True
        if find_parent_and_add(btn.get("children", []), parent_id, new_btn):
            return True
    return False
...
def delete_application(app_id):
    ...
    for i, a in enumerate(data.get("applications", [])):
        if a["id"] == app_id:
            ...
            data["stats"][old_status]  = max(0, data["stats"].get(old_status, 0) - 1)
            save_data(data)
            return jsonify({"success": True})
    return jsonify({"error": "Topilmadi"}), 404
...
def update_welcome():
    config = load_config()
    config["welcome_message"] = request.json.get("message", config["welcome_message"])
    save_config(config)
    return jsonify({"success": True})
...
def add_button():
    ...
    new_btn = {
        "id": str(uuid.uuid4())[:8],
        "label": (d.get("icon","") + " " + d.get("text","")).strip(),
        "icon": d.get("icon",""), "text": d.get("text","Yangi tugma"),
        "type": d.get("type","message"), "message": d.get("message",""),
        "section": d.get("section",""), "children": []
    }
    parent_id = d.get("parent_id","")
    if parent_id:
        find_parent_and_add(config["buttons"], parent_id, new_btn)
    else:
...
def update_button(btn_id):
    config = load_config()
    d = request.json
    icon, text = d.get("icon",""), d.get("text","")
    update = {"icon": icon, "text": text, "label": (icon+" "+text).strip(),
              "type": d.get("type","message"), "message": d.get("message",""), "section": d.get("section","")}
    if find_and_update(config["buttons"], btn_id, update):
        save_config(config)
        return jsonify({"success": True})
    return jsonify({"error": "Topilmadi"}), 404
...
bot_loop = None   # asyncio event loop — flask reply uchun
...
async def _send_msg(user_id, text):
    try:
        await tg_bot.send_message(user_id, text)
...
def save_application(user, section, detail, name, info, phone):
    ...
    app = {
        ...
        "username": user.username or "", "name": name, "info": info,
        "phone": phone, "section": section, "detail": detail,
        "time": datetime.now().strftime("%H:%M, %d-%B"), "status": "pending", "replies": []
    }
...
def save_contact(user, text, phone):
    data = load_data()
    contact = {
        "id": len(data["contacts"]) + 1, "user_id": user.id,
        "tg_name": f"{user.first_name or ''} {user.last_name or ''}".strip(),
...
async def get_name(message: types.Message, state: FSMContext):
    ...
    await state.set_state(AppForm.waiting_info)
...
async def handle_any(message: types.Message, state: FSMContext):
        ...
        if parent and parent.get("children"):
            for child in parent["children"]:
                if child.get("label") == message.text:
                    btn = child; break
    if not btn:
        btn = find_by_label(config["buttons"], message.text)
    if not btn: return
    ...
    if children:
        ...
        if "haqimizda" in btn.get("text","").lower() and not current_menu_id:
            ...
            try:
                await message.answer_photo(photo=ABOUT_IMAGE, caption=txt, reply_markup=channel_btn); return
            except:
                await message.answer(txt, reply_markup=channel_btn); return
        await message.answer(txt)


# ═══════════════════════════════════════════════════════════════════════
# START — Bot va Flask birgalikda
# ═══════════════════════════════════════════════════════════════════════
