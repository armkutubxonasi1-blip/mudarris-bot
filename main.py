...
# ── Keep-alive: server uyquga ketmasin ────────────────────────────────
APP_URL = os.environ.get("RAILWAY_PUBLIC_DOMAIN", "")

async def keep_alive():
    """Har 4 daqiqada o'ziga ping — Railway cold start bo'lmasin"""
    await asyncio.sleep(30)  # bot ishga tushgandan keyin kut
    while True:
        try:
            if APP_URL:
                req_lib.get(f"https://{APP_URL}/", timeout=5)
                print("🏓 Keep-alive ping")
        except:
            pass
        await asyncio.sleep(240)  # 4 daqiqa

# ── In-memory cache — diskka har so'rovda bormasin ────────────────────
_data_cache   = None
_config_cache = None
_cache_lock   = threading.Lock()

_orig_load_data   = load_data
_orig_load_config = load_config
_orig_save_data   = save_data
...
def save_config(config):
    ...
    with _cache_lock:
        ...
        _orig_save_config(config)

# ── Combined API endpoint — 3 so'rov o'rniga 1 ta ─────────────────────
